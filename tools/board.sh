...
#
# Takes OVER TEN MINUTES; `lethality` is about eight of them on its own. The
# "about 5 minutes" this line used to claim was measured wrong and blew through a
# 600 s timeout on 2026-08-16. Run it in the background and expect to wait.
#
# DO NOT EDIT THIS FILE WHILE IT IS RUNNING - not even a comment. Bash reads a
# script incrementally by BYTE OFFSET as it executes, so inserting these three
# lines during a run shifted everything below the read position and the shell
# resumed mid-line: 27 checks passed, then the script died on a phantom
# "unexpected EOF" pointing at a bare `echo`, and returned an exit code this
# script cannot produce. Ten minutes is long enough that tidying something while
# you wait feels free. It is not. (2026-08-18)
#
