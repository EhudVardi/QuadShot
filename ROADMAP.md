...
- **Arcade mode** *(later, from design doc v1.7)*: a dedicated mode based on and expanding the dev room — grows as the game grows (terrain, enemies, weapons); may eventually replace the dev room outright.
- **Interactive tutoring** *(late-cycle, from design doc v1.6.1)*: opt-outable in-gameplay guidance — on first encounter with a new mechanic, slow/freeze time (the pause/slow-mo + muffle machinery is literally the delivery vehicle), blur the world, and give one concise hint, ideally as a visual cue rather than text. Deferred until the systems it must teach are themselves stable.
